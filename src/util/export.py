    ...
    with zipfile.ZipFile(archive_path, 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
        ...
        for i, file in enumerate(files, start=1):
            ...
            if file.name == config.logging.filename:
                m = re.match(r'^(.+)_part(\d+)\.log$', file.name)
                new_name = f"{m.group(1)}_part{int(m.group(2)) + 1}.log" if m else f"{file.stem}_part2.log"
                new_log_file = config.logging.directory / new_name
                archived_name = f"{file.stem}_part1.log" if not m else file.name
                LogMessage(f"About to archive active log file - app is still running despite this being penultimate log entry\nFurther logs will be saved to a new file: {new_log_file} (not included in this archive)\nThis log file is renamed to {archived_name}", level = "WARNING")
                config.logging.filename = new_name
                archived_name = f"{file.stem}_part1.log" if not m else file.name
                os.rename(file, config.logging.directory / archived_name)
                file = config.logging.directory / archived_name
...
def export_logs(remove_original = False) -> Path | None:
    ...
    files = [config.logging.directory / file for file in os.listdir(config.logging.directory)] # Refresh file list after potential renaming of active log file
