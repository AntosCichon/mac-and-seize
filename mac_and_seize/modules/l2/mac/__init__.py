...
Exposes the ``l2 mac`` command surface (``flood`` / ``stop``) and the
session-scoped :class:`~mac_and_seize.modules.l2.mac.service.MacFloodService`.
...
from mac_and_seize.modules.l2.mac.actions import (
    GROUP_DESCRIPTIONS,
    SERVICE,
    build_actions,
)
